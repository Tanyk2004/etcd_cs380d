...
while true; do
    ...
    # ── noise-node ops/s + stall stats: scrape /health on all 3 node ports ─────
    # Returns {"node":...,"ops":N,"stall_count":N,"stall_ms_total":N,...}
    # One curl per node; parse all needed fields from the single response.
    nn1_h=$(curl -s --max-time 1 localhost:19001/health 2>/dev/null)
    nn2_h=$(curl -s --max-time 1 localhost:19002/health 2>/dev/null)
    nn3_h=$(curl -s --max-time 1 localhost:19003/health 2>/dev/null)

    nn1=$(printf '%s' "$nn1_h" | grep -o '"ops":[0-9]*'            | cut -d: -f2)
    nn2=$(printf '%s' "$nn2_h" | grep -o '"ops":[0-9]*'            | cut -d: -f2)
    nn3=$(printf '%s' "$nn3_h" | grep -o '"ops":[0-9]*'            | cut -d: -f2)
    ...
    nn1_sc=$(printf '%s'  "$nn1_h" | grep -o '"stall_count":[0-9]*'    | cut -d: -f2)
    nn2_sc=$(printf '%s'  "$nn2_h" | grep -o '"stall_count":[0-9]*'    | cut -d: -f2)
    nn3_sc=$(printf '%s'  "$nn3_h" | grep -o '"stall_count":[0-9]*'    | cut -d: -f2)
    stall_count=$(awk -v a="${nn1_sc:-0}" -v b="${nn2_sc:-0}" -v c="${nn3_sc:-0}" \
        'BEGIN{print a+b+c}')

    nn1_sms=$(printf '%s' "$nn1_h" | grep -o '"stall_ms_total":[0-9]*' | cut -d: -f2)
    nn2_sms=$(printf '%s' "$nn2_h" | grep -o '"stall_ms_total":[0-9]*' | cut -d: -f2)
    nn3_sms=$(printf '%s' "$nn3_h" | grep -o '"stall_ms_total":[0-9]*' | cut -d: -f2)
    stall_ms_total=$(awk -v a="${nn1_sms:-0}" -v b="${nn2_sms:-0}" -v c="${nn3_sms:-0}" \
        'BEGIN{print a+b+c}')
    ...
    echo "$timestamp,${leader_changes:-0},${proposals_pending:-0},${proposals_failed:-0},${peer_rtt:-0},${hb_failures:-0},$wal_avg_ms,$bck_avg_ms,${has_leader:-1},${proposals_committed:-0},${slow_apply:-0},${peer_sent_bytes:-0},${noise_ops_s},${tc_drops_s},${tc_overlimits_s},${tc_backlog_bytes:-0},${tc_backlog_pkts:-0},${stall_count:-0},${stall_ms_total:-0}" \
        >> "$OUTPUT_DIR/timeseries.csv"
