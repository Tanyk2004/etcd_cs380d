...
func main() {
	...
	go func() {
		...
		prevU1, prevU10, prevU100, prevSlow := int64(0), int64(0), int64(0), int64(0)
		...
		for {
			...
			fmt.Printf(
				"[stats] ts=%-10d profile=%-15s ops/s=%-6d put=%-5d get=0     del=0     txn=0     lease=0     err=%-4d | lat(u1ms=%d u10ms=%d u100ms=%d slow=%d)\n",
				time.Now().Unix(), *profileFlag, dOps, dOps-dErrs, dErrs,
				dU1, dU10, dU100, dSlow,
			)

			prevOps, prevErrs = ops, errs
			prevU1, prevU10, prevU100, prevSlow = u1, u10, u100, slow
		}
	}()
		...
		go func(workerID int) {
			...
			for {
				...
				if err != nil {
					...
					atomic.AddInt64(&latSlow, 1) // errors are always "slow"
					continue
				}
				resp.Body.Close()
